// Dump decompiled caller functions for the supplied callee addresses.
//
// Usage (headless postScript):
//   DumpCallersByAddress.java <outputDir> <hexAddress> [<hexAddress> ...]
//
//@category Bomberman
...
public class DumpCallersByAddress extends GhidraScript {

	@Override
	protected void run() throws Exception {
		String[] args = getScriptArgs();
		if (args.length < 2) {
			println("Usage: DumpCallersByAddress.java <outputDir> <hexAddress> [<hexAddress> ...]");
			return;
		}
		...
		File indexFile = new File(outDir, "caller_refs.tsv");
		try (PrintWriter indexWriter = new PrintWriter(new FileWriter(indexFile))) {
			indexWriter.println("target\tfrom\tref_type\tcaller_entry\tcaller_name");
			for (int i = 1; i < args.length; i++) {
				collectCallersForAddress(args[i], uniqueCallers, indexWriter);
			}
		}
		...
		try {
			for (Function function : uniqueCallers.values()) {
	...
	private void collectCallersForAddress(String addressText, Map<String, Function> uniqueCallers,
			PrintWriter indexWriter) {
		Address target = toAddr(addressText);
		if (target == null) {
			printf("Could not parse address: %s%n", addressText);
			return;
		}
		...
		for (Reference reference : refs) {
			...
			uniqueCallers.put(caller.getEntryPoint().toString(), caller);
			indexWriter.printf("%s\t%s\t%s\t%s\t%s%n", target, from, reference.getReferenceType(),
